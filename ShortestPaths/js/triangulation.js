...
import { Vertex, create_full_edge } from "../../_Common/polygon.js";
import { p_orientation } from "../../_Common/utils.js";
const EPSILON = 1E-4;
const VERTEX_TYPE = {
    START: 0,
    END: 1,
...
function vertex_cmp(u, v) {
    if (Math.abs(u.y - v.y) < EPSILON)
        return u.x < v.x;
    return u.y < v.y;
}
function vertex_num_cmp(u, v) {
...
function half_edge_cmp(h_0, h_1) {
...
export class Triangulator {
    constructor() {
        this.query_edges = create_full_edge(new Vertex(0, 0), new Vertex(1, 1));
        this.tree = new BinaryTree(half_edge_cmp);
    ...
    triangulate_monotone(chain_data) {
        const stack = [];
        ...
        while (p_queue.length > 0) {
            ...
            switch (type) {
                case VERTEX_TYPE.START:
                    this.handle_start_vertex(v);
                    break;
                case VERTEX_TYPE.END:
                    this.handle_end_vertex(v);
                    break;
                case VERTEX_TYPE.SPLIT:
                    this.handle_split_vertex(v);
                    break;
                case VERTEX_TYPE.MERGE:
                    this.handle_merge_vertex(v);
                    break;
                case VERTEX_TYPE.REGULAR:
                    this.handle_regular_vertex(v);
                    break;
            }
        }
    }
...
function top_to_down_edge(edge) {
    return edge.origin.y > edge.destination().y ? edge : edge.twin;
}
function determine_vertex_type(v) {
